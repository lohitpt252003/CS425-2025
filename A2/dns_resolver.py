...
def recursive_dns_lookup(domain):
    ...
    First, it fetches the NS records, then the A record(s) for the domain.
    """
    print(f"[Recursive DNS Lookup] Resolving {domain}")
    try:
        # Resolve NS records and print them
        ns_answer = dns.resolver.resolve(domain, "NS")
        for rdata in ns_answer:
            ns_name = rdata.to_text()
            # Ensure trailing period (if required by your expected format)
            if not ns_name.endswith('.'):
                ns_name += '.'
            print(f"[SUCCESS] {domain} -> {ns_name}")
        
        # Resolve A records and print them
